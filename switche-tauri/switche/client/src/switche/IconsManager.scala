...
object IconsManager {
   ...
   private def initNodeIconExtractor() = {
      //NodeIconExtractor.registerHwndIconStringCallback (nodeIconExtractorHwndIconStringCallback _)
   }
   ...
   private def hwndExePathPair (e:WinDatEntry) = {
      e.exe_path_name.map(_.full_path) .map (p => HwndExePathPair(e.hwnd,p))
   }
   def getCachedIcon (e:WinDatEntry) : Option[String] = {
      hwndExePathPair(e).flatMap {hpp =>
   ...
   def markCachedIconMappingsStale () = { SwitcheState.hwndMap .valuesIterator .foreach (markCachedIconMappingStale) }
