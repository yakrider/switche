...
object SearchHelper {
   ...
   def checkSearch (str:String, chkFrags:Seq[String]):Span = {
   ...
   def checkSearchText (str:String, chkFrags:Seq[String]) = {
      ...
      if (chkPassed) { CheckSearchTextRes (true, checkSearch(str,chkFrags))
   ...
   def checkSearchExeTitle (exeName:String, title:String, toMatch:String) : CheckSearchExeTitleRes = {
      ...
      // for exe match to be true, only one of the terms need to match exe, BUT, if there are other terms, those must match in title
      ...
      val didExeMatch = exeMatchChecks.exists(_._2)
      val didTitleMatch = chkFrags.forall(titleLower.contains)
      val didExeTitleMatch = exeMatchChecks.filterNot(_._2).map(_._1).forall(titleLower.contains)
      // note for checks, that forall on empty will return true, hence gotta check both didExeMatch and didExeTitleMatch
      
      if ((didExeMatch && didExeTitleMatch) || didExeTitleMatch) {
         val exeSpan = exeMatchChecks.filter(_._2).headOption.map(_._1).map(f => checkSearch(exeName,Seq(f))).getOrElse(span(exeName).render)
         val titleSpan = checkSearch (title,chkFrags)
